...
impl ProtectiveMeasuresQueries {
    pub const CREATE_PROTECTIVE_MEASURE: &'static str = r#"
        ...
        RETURNING id, process_number, sei_process_number, occurrence_report_number, issued_at, judicial_authority, court_district_id, distance_meters, status, violence_types, relationship_to_victim, assaults_children, was_drunk_during_assault, victim_id, offender_id, created_at, updated_at, is_deleted
    "#;
    ...
    pub const GET_PROTECTIVE_MEASURES_PAGED_BY_CITIES: &'static str = r#"
        SELECT pm.id, pm.process_number, pm.sei_process_number, pm.occurrence_report_number, pm.issued_at, pm.judicial_authority, pm.court_district_id, pm.distance_meters, pm.status, pm.violence_types, pm.relationship_to_victim, pm.assaults_children, pm.was_drunk_during_assault, pm.victim_id, pm.offender_id, pm.created_at, pm.updated_at, pm.is_deleted
    ...
    pub const UPDATE_PROTECTIVE_MEASURE_BY_ID: &'static str = r#"
        ...
        SET process_number = $2, sei_process_number = $3, occurrence_report_number = $4, issued_at = $5, judicial_authority = $6, court_district_id = $7, distance_meters = $8, status = $9, violence_types = $10, relationship_to_victim = $11, assaults_children = $12, was_drunk_during_assault = $13, victim_id = $14, offender_id = $15
        WHERE id = $1 AND is_deleted = false
        RETURNING id, process_number, sei_process_number, occurrence_report_number, issued_at, judicial_authority, court_district_id, distance_meters, status, violence_types, relationship_to_victim, assaults_children, was_drunk_during_assault, victim_id, offender_id, created_at, updated_at, is_deleted
    "#;
