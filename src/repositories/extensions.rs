...
impl ExtensionRepository for PgExtensionRepository {
    async fn create_extension(&self, protective_measure_id: Uuid, extension: CreateExtension) -> Result<ProtectiveMeasureExtension, sqlx::Error> {
        ...
        info!("[Repository] Executing SQL query to create extension {} for protective measure: {}", extension.extension_number, protective_measure_id);
        ...
        info!("[Repository] Extension successfully inserted into database with ID: {}", extension_created.id);

        Ok(extension_created)
    }

    async fn get_extension_by_id(&self, id: Uuid) -> Result<ProtectiveMeasureExtension, sqlx::Error> {
        info!("[Repository] Executing SQL query to get extension with id: {}", id);

        let extension: ProtectiveMeasureExtension = sqlx::query_as(ExtensionsQueries::GET_EXTENSION_BY_ID)
            .bind(id)
            .fetch_one(&self.pool)
            .await?;

        info!("[Repository] Extension successfully found in the database with ID: {}", id);

        Ok(extension)
    }

    async fn get_extensions_by_measure(&self, protective_measure_id: Uuid) -> Result<Vec<ProtectiveMeasureExtension>, sqlx::Error> {
        info!("[Repository] Executing SQL query to get extensions for protective measure: {}", protective_measure_id);

        let extensions: Vec<ProtectiveMeasureExtension> = sqlx::query_as(ExtensionsQueries::GET_EXTENSIONS_BY_MEASURE)
            .bind(protective_measure_id)
            .fetch_all(&self.pool)
            .await?;

        info!("[Repository] Found {} extensions for protective measure: {}", extensions.len(), protective_measure_id);
    ...
    async fn get_all_extensions(&self) -> Result<Vec<ProtectiveMeasureExtension>, sqlx::Error> {
        ...
        let extensions: Vec<ProtectiveMeasureExtension> = sqlx::query_as(ExtensionsQueries::GET_ALL_EXTENSIONS)
            .fetch_all(&self.pool)
            .await?;

        info!("[Repository] Found {} extensions in database", extensions.len());

        Ok(extensions)
    }

    async fn update_extension_by_id(&self, data: UpdateExtension, id: Uuid) -> Result<ProtectiveMeasureExtension, sqlx::Error> {
        info!("[Repository] Executing SQL query to update extension with ID: {}", id);
        ...
        info!("[Repository] Extension successfully updated in database with ID: {}", id);

        Ok(extension_updated)
    }

    async fn delete_extension_by_id(&self, id: Uuid) -> Result<ProtectiveMeasureExtension, sqlx::Error> {
        info!("[Repository] Executing SQL query to soft delete extension with id: {}", id);

        let deleted_extension: ProtectiveMeasureExtension = sqlx::query_as(ExtensionsQueries::DELETE_EXTENSION_BY_ID)
            .bind(id)
            .fetch_one(&self.pool)
            .await?;

        info!("[Repository] Extension successfully soft deleted from database with ID: {}", id);
