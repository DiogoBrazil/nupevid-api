...
pub async fn add_member(
    ...
    let session_id = path.into_inner();
    info!("[Controller] Received request to add member to session: {}", session_id);
    service.add_member_to_session(session_id, data.user_id, data.function.clone(), req).await
}
...
pub async fn remove_member(
    ...
    info!("[Controller] Received request to remove member {} from session: {}", member_id, session_id);
    service.remove_member_from_session(session_id, member_id, req).await
}
