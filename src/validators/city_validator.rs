    ...
    pub fn validate_fields(
        ...
        if !is_valid_city_name(name) {
            return Err(AppError::BadRequest(format!(
                "{}: invalid city name '{}'. Valid cities: {:?}",
                error_context, name, VALID_CITIES
            )));
        }
        ...
        if !is_valid_state(state) {
            return Err(AppError::BadRequest(format!(
                ...
                error_context, state, VALID_STATES
            )));
        }

        if !is_valid_battalion(battalion) {
            return Err(AppError::BadRequest(format!(
                "{}: invalid battalion '{}'. Valid battalions: {:?}",
                error_context, battalion, VALID_BATTALIONS
            )));
        }
...
mod tests {
    ...
    fn test_validate_fields_success() {
        let result = CityValidator::validate_fields("PORTO VELHO", "RO", "1ºBPM", "test");
        assert!(result.is_ok());
    }
    ...
    fn test_validate_fields_empty_name() {
        let result = CityValidator::validate_fields("", "RO", "1ºBPM", "test");
    ...
    fn test_validate_fields_invalid_city_name() {
        let result = CityValidator::validate_fields("INVALID CITY", "RO", "1ºBPM", "test");
        assert!(result.is_err());
        assert!(
            result
                .unwrap_err()
    ...
    fn test_validate_fields_invalid_state() {
        let result = CityValidator::validate_fields("PORTO VELHO", "SP", "1ºBPM", "test");
        assert!(result.is_err());
    ...
    fn test_validate_fields_invalid_battalion() {
        let result = CityValidator::validate_fields("PORTO VELHO", "RO", "INVALID", "test");
        assert!(result.is_err());
        assert!(
            result
                .unwrap_err()
