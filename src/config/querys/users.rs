...
impl UsersQueries {
    pub const CREATE_USER: &'static str = r#"
        INSERT INTO users (id, rank, registration, full_name, profile, email, password, city_id)
        VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
        RETURNING id, rank, registration, full_name, profile, email, city_id, created_at, updated_at, is_deleted
    "#;

    pub const GET_USER_BY_ID: &'static str = r#"
        SELECT id, rank, registration, full_name, profile, email, city_id, created_at, updated_at, is_deleted
        FROM users
    ...
    pub const UPDATE_USER_BY_ID: &'static str = r#"
        UPDATE users
        SET rank = $2, registration = $3, full_name = $4, profile = $5, email = $6, city_id = $7
        WHERE id = $1 AND is_deleted = false
        RETURNING id, rank, registration, full_name, profile, email, city_id, created_at, updated_at, is_deleted
    "#;
