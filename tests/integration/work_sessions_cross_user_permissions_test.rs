...
async fn city_user_cannot_view_other_city_user_session() {
    ...
    let user_a = db_fixtures::insert_user(&pool, "700001", "user_a@test.com", "CITY_USER", Some(city)).await;
    let user_b = db_fixtures::insert_user(&pool, "700002", "user_b@test.com", "CITY_USER", Some(city)).await;
...
async fn city_admin_can_view_other_user_session_in_same_city() {
    ...
    let city = db_fixtures::insert_city(&pool, "Test City").await;
    let user = db_fixtures::insert_user(&pool, "700003", "user@test.com", "CITY_USER", Some(city)).await;
    let admin = db_fixtures::insert_user(&pool, "700004", "admin@test.com", "CITY_ADMIN", Some(city)).await;
...
async fn user_can_view_own_session() {
    ...
    let city = db_fixtures::insert_city(&pool, "Test City").await;
    let user_id = db_fixtures::insert_user(&pool, "700005", "user@test.com", "CITY_USER", Some(city)).await;
