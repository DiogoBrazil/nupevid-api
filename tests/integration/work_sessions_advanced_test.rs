...
async fn session_members_from_different_cities_fails() {
    ...
    let creator_id = db_fixtures::insert_user(&pool, "100001", "creator@test.com", "CITY_ADMIN", Some(city_a)).await;
    let member_from_city_b = db_fixtures::insert_user(&pool, "100002", "member@test.com", "CITY_USER", Some(city_b)).await;
...
async fn non_creator_cannot_add_members() {
    ...
    let other_user_id = db_fixtures::insert_user(&pool, "100004", "other@test.com", "CITY_USER", Some(city)).await;
    let new_member_id = db_fixtures::insert_user(&pool, "100005", "newmember@test.com", "CITY_USER", Some(city)).await;
...
async fn user_in_active_session_cannot_join_another() {
    ...
    let city = db_fixtures::insert_city(&pool, "Test City").await;
    let creator1_id = db_fixtures::insert_user(&pool, "100006", "creator1@test.com", "CITY_ADMIN", Some(city)).await;
    let creator2_id = db_fixtures::insert_user(&pool, "100007", "creator2@test.com", "CITY_USER", Some(city)).await;
    let shared_member_id = db_fixtures::insert_user(&pool, "100008", "shared@test.com", "CITY_USER", Some(city)).await;
    ...
    let body: serde_json::Value = test::read_body_json(add_resp).await;
    assert!(body["message"].as_str().unwrap().contains("already in an active session"));
}
...
async fn root_can_view_any_session() {
    ...
    let city = db_fixtures::insert_city(&pool, "Test City").await;
    let creator_id = db_fixtures::insert_user(&pool, "100009", "creator@test.com", "CITY_ADMIN", Some(city)).await;
...
async fn city_admin_cannot_view_other_city_session() {
    ...
    let creator_a_id = db_fixtures::insert_user(&pool, "100011", "creator_a@test.com", "CITY_ADMIN", Some(city_a)).await;
    let admin_b_id = db_fixtures::insert_user(&pool, "100012", "admin_b@test.com", "CITY_ADMIN", Some(city_b)).await;
