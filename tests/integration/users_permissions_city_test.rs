...
async fn city_admin_cannot_get_user_from_other_city() {
    ...
    assert_eq!(get_user_resp.status(), StatusCode::FORBIDDEN, "CITY_ADMIN should not access users from other cities");
}
...
async fn city_admin_cannot_update_user_from_other_city() {
    ...
    let update_req = test_helpers::with_auth_headers(
        test::TestRequest::put().uri(&format!("/api/v1/users/{}", user2_id)).set_json(&update_payload),
        &config, &admin_token).to_request();
    let update_resp = test::call_service(&app, update_req).await;

    assert_eq!(update_resp.status(), StatusCode::FORBIDDEN, "CITY_ADMIN should not update users from other cities");
}
...
async fn city_admin_cannot_delete_user_from_other_city() {
    ...
    assert_eq!(delete_resp.status(), StatusCode::FORBIDDEN, "CITY_ADMIN should not delete users from other cities");
}
