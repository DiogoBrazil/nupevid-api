pub mod attendance_filter_by_measure_test;
pub mod attendance_members_test;
pub mod attendance_offenders_integration_test;
pub mod attendance_victims_integration_test;
pub mod attendance_victims_validation_test;
pub mod auth_login_test;
pub mod auth_refresh_test;
pub mod auth_validation_test;
pub mod bug_fixes_regression_test;
pub mod cities_integration_test;
pub mod concurrency_invariants_test;
pub mod offenders_basic_test;
pub mod offenders_phones_addresses_crud_test;
pub mod pagination_test;
pub mod policies_integration_test;
pub mod protective_measures_include_related_test;
pub mod protective_measures_integration_test;
pub mod protective_measures_state_transitions_test;
pub mod protective_measures_update_test;
pub mod route_policy_matrix;
pub mod soft_delete_sweep_test;
pub mod structural_routes_test;
pub mod users_create_test;
pub mod users_multicity_test;
pub mod users_password_security_test;
pub mod users_password_test;
pub mod users_permissions_city_test;
pub mod users_read_test;
pub mod users_search_test;
pub mod users_update_delete_test;
pub mod users_validation_test;
pub mod victims_integration_test;
pub mod victims_phones_addresses_test;
pub mod victims_validation_test;
pub mod work_sessions_additional_edge_cases_test;
pub mod work_sessions_advanced_test;
pub mod work_sessions_commander_permissions_test;
pub mod work_sessions_cross_user_permissions_test;
pub mod work_sessions_data_integrity_test;
pub mod work_sessions_edge_cases_test;
pub mod work_sessions_error_test;
pub mod work_sessions_include_related_test;
pub mod work_sessions_input_validation_test;
pub mod work_sessions_new_endpoints_test;
pub mod work_sessions_nonexistent_users_test;
pub mod work_sessions_root_permissions_test;
pub mod work_sessions_team_management_test;
pub mod work_sessions_test;
pub mod work_sessions_update_full_test;
